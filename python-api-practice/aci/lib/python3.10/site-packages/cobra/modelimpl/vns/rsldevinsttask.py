...
# ##################################################
class RsLDevInstTask(Mo):
    """
    A source relation to the local state in the object that reuses encaps across graphs with the same subnet. 

    """

    meta = ClassMeta("cobra.model.vns.RsLDevInstTask")

    meta.moClassName = "vnsRsLDevInstTask"
    meta.rnFormat = "vnsRsLDevInstTask-%(id)s"
    ...
    prop = PropMeta("str", "id", "id", 15515, PropCategory.REGULAR)
    prop.label = "ID"
    prop.isConfig = True
    prop.isAdmin = True
    prop.isCreateOnly = True
    ...
    prop._addConstant("VDevUpd", "vdevupd", 1273)
