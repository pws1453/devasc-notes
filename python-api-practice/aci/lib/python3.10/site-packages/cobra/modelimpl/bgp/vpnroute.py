...
class VpnRoute(Mo):
    """
    The BGP route table for a VPN address family (VPNv4 unicast and VPNv6 unicast). The VPN address family routes are exchanged within the fabric over MP-BGP sessions between spines and leafs.  Routes are advertised from border leafs to non-border leafs with spines acting as route reflectors.

    """

    meta = ClassMeta("cobra.model.bgp.VpnRoute")

    meta.moClassName = "bgpVpnRoute"
    meta.rnFormat = "vpnrt-[%(pfx)s]-%(rd)s"
    meta.category = MoCategory.REGULAR
    meta.label = "VPN Route"
    ...
    meta.childClasses.add("cobra.model.bgp.Path")

    meta.childNamesAndRnPrefix.append(("cobra.model.bgp.Path", "path-"))
    ...
    prop = PropMeta("str", "pfx", "pfx", 5889, PropCategory.REGULAR)
    prop.label = "Prefix"
    prop.isConfig = True
    prop.isAdmin = True
    prop.isCreateOnly = True
    ...
    prop = PropMeta("str", "rd", "rd", 5888, PropCategory.REGULAR)
    ...
    def __init__(self, parentMoOrDn, pfx, rd, markDirty=True, **creationProps):
        namingVals = [pfx, rd]
