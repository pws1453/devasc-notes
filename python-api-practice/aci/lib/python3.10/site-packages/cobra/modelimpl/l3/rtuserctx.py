...
class RtUserCtx(Mo):
    """
    A target relation to tenant context information. The tenant context is equivalent to a virtual routing and forwarding (VRF) instance created for the tenant's L3 network. Similar to a VRF in traditional Cisco routers, the tenant context isolates the IP addresses of the tenant, allowing different tenants to have overlapping IP addresses.

    """

    meta = TargetRelationMeta("cobra.model.l3.RtUserCtx", "cobra.model.dbgexp.ExportP")

    meta.moClassName = "l3RtUserCtx"
    meta.rnFormat = "rtdbgexpUserCtx-[%(tDn)s]"
    meta.category = MoCategory.RELATIONSHIP_FROM_GLOBAL
    meta.label = "Abstract Class for Export Policy"
    meta.writeAccessMask = 0x400000000001
    meta.readAccessMask = 0x708401002101
    ...
    prop = PropMeta("str", "parentSKey", "parentSKey", 15170, PropCategory.REGULAR)
    prop.label = "None"
    prop.isImplicit = True
    prop.isAdmin = True
    prop.range = [(0, 16)]
    ...
    prop = PropMeta("str", "tCl", "tCl", 12954, PropCategory.REGULAR)
    prop.label = "Target-class"
    prop.isImplicit = True
    prop.isAdmin = True
    prop.defaultValue = 4104
    prop.defaultValueStr = "dbgexpExportP"
    ...
    prop = PropMeta("str", "tDn", "tDn", 12953, PropCategory.REGULAR)
