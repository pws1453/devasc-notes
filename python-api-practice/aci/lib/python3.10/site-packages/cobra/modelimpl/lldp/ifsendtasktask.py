...
# ##################################################
class IfSendTaskTask(Mo):
    """
    Information returned by or provided to a task's task.

    """

    meta = ClassMeta("cobra.model.lldp.IfSendTaskTask")

    meta.moClassName = "lldpIfSendTaskTask"
    meta.rnFormat = "lldpIfSendTaskTask-%(id)s"
    ...
    prop = PropMeta("str", "id", "id", 5667, PropCategory.REGULAR)
    prop.label = "ID"
    prop.isConfig = True
    prop.isAdmin = True
    prop.isCreateOnly = True
    ...
    prop._addConstant("send", "send", 666)
