...
# ##################################################
class RsFuncToEpgTask(Mo):
    """
    A task to send hypervisor function information from one process to another. Note that this relation is an internal object.

    """

    meta = ClassMeta("cobra.model.infra.RsFuncToEpgTask")

    meta.moClassName = "infraRsFuncToEpgTask"
    meta.rnFormat = "infraRsFuncToEpgTask-%(id)s"
    ...
    prop = PropMeta("str", "id", "id", 5696, PropCategory.REGULAR)
    prop.label = "ID"
    prop.isConfig = True
    prop.isAdmin = True
    prop.isCreateOnly = True
    ...
    prop._addConstant("ReportPortsToEpp", "reportportstoepp", 702)
