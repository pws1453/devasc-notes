...
class ConsLbl(Mo):
    """
    A label used by consumers to filter the providers. The label can be parented as follows:<ul> <li>By the consumer endpoint group.</li> <li>By the relation between the consumer endpoint group and contract.</li> <li>By the relation between the contract interface and contract.</li> <li>By the relation between the consumer endpoint group and contract interface.</li> </ul> A consumer with no label will consume from all the providers of the contract with no labels. A consumer with a specific label can only consume from providers matching the label.

    """

    meta = ClassMeta("cobra.model.vz.ConsLbl")

    meta.moClassName = "vzConsLbl"
    meta.rnFormat = "conslbl-%(name)s"
    meta.category = MoCategory.REGULAR
    meta.label = "Consumer Label"
    ...
    prop = PropMeta("str", "annotation", "annotation", 37479, PropCategory.REGULAR)
    ...
    prop = PropMeta("str", "extMngdBy", "extMngdBy", 39618, PropCategory.REGULAR)
    prop.label = "None"
    prop.isImplicit = True
    prop.isAdmin = True
    prop.defaultValue = 0
    ...
    prop = PropMeta("str", "name", "name", 7411, PropCategory.REGULAR)
    prop.label = "Name"
    prop.isConfig = True
    prop.isAdmin = True
    prop.isCreateOnly = True
